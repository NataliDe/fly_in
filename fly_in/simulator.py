...
class Simulator:
    ...
    def step(self) -> None:
        ...
        for drone in self.drones:
            ...
            if drone.remaining_turns <= 0 and drone.to_hub is not None and drone.from_hub is not None:
        ...
        idle_drones = [d for d in self.drones if not d.finished and not d.is_moving]
        idle_drones.sort(
            key=lambda drone: (
        ...
        for drone in idle_drones:
            ...
            for hub_name, hub in self.map_data.hubs.items():
                if hub.kind == "end":
                    continue
                if occupancy[hub_name] + incoming[hub_name] + reserved_targets[hub_name] >= hub.effective_capacity():
                    blocked_hubs.add(hub_name)

            blocked_links = set()
            for key, conn in self.map_data.connections.items():
                if link_load[key] + reserved_links[key] >= conn.max_link_capacity:
            ...
            candidate_names = [item.next_hub for item in ranked if item.score <= ranked[0].score + 0.20]
            preferred_neighbors = self._preferred_neighbors(drone.current_hub, candidate_names)
            ...
            future_occ = occupancy[next_hop] + incoming[next_hop] + reserved_targets[next_hop]
            if target_hub.kind != "end" and future_occ >= target_hub.effective_capacity():
                continue
            if link_load[conn.key] + reserved_links[conn.key] >= conn.max_link_capacity:
                continue
            ...
            self._remember_dispatch(drone.current_hub, next_hop, candidate_names)
